/**
 * Importació de funcions - Servidor
 */
...
//import { login, verificarSessio } from "./login.js";
...
document.addEventListener("DOMContentLoaded", function () {
  carregarComponent("header");
  carregarComponent("footer");
  carregarComponent("headerPetit");
  carregarComponent("footerPetit");
});

/**
 * Funció que carrega un component HTML al DOM des d'un fitxer extern.
 *
 * @param {string} component - El nom del component a carregar, que també s'utilitza com a ID
 * de l'element al DOM. S'espera el fitxer HTML corresponent a `./components/{component}.html`.
 * @throws {Error} Si no es pot carregar el fitxer HTML.
 */
function carregarComponent(component) {
    ...
    fetch(`./components/${component}.html`)
      ...
      .then((html) => {
        ...
        if (component === "header") {
          ...
          tancarSessio();
          window.tornarEnrere = tornarEnrere; //Assigna tornarEnrere a window.
        }
      })
      .catch((error) => {
